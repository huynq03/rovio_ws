...
echo "===== START ROVIO ODOM -> FLU AXIS CONVERTER ====="
ros2 run rovio_qos_relay rovio_odom_to_flu \
  > /tmp/rovio_odom_to_flu.log 2>&1 &

FLU_PID=$!

cleanup() {
  echo
  echo "===== CLEANUP D435I ROVIO FLU PIPELINE ====="

  kill "$FLU_PID" 2>/dev/null || true
  kill "$ROVIO_PID" 2>/dev/null || true
  kill "$RELAY_PID" 2>/dev/null || true
  kill "$RS_LAUNCH_PID" 2>/dev/null || true

  FLU_PIDS=$(ps -eo pid,cmd | awk '/[r]ovio_odom_to_flu/ {print $1}')
  if [ -n "$FLU_PIDS" ]; then
    echo "Killing FLU converter PID(s): $FLU_PIDS"
    kill $FLU_PIDS 2>/dev/null || true
  fi
...
echo "Raw ROVIO odom:     /rovio/odometry"
echo "FLU converted odom: /rovio/odometry_flu"
echo
echo "Check:"
echo "  source /opt/ros/humble/setup.bash && source ~/rovio_ws/install/setup.bash && ros2 topic echo /rovio/odometry_flu"
echo
